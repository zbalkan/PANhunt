        ...
        if not hasattr(self, '_initialized'):  # Ensures __init__ runs only once
            ...
            self.__pattern_list = [
                ('Mastercard', re.compile(
                    r'r"(?<![A-Za-z0-9_])((?:5[1-5]\d{2}|222[1-9]|22[3-9]\d|2[3-6]\d{2}|27[01]\d|2720)[ -]?\d{4}[ -]?\d{4}[ -]?\d{4})(?!(?:[ -]?\d)|[A-Za-z0-9_])"',
                    flags=default_flag)),
                ('Visa', re.compile(
                    r'r"(?<![A-Za-z0-9_])(4\d{3}[ -]?\d{4}[ -]?\d{4}[ -]?\d{4})(?!(?:[ -]?\d)|[A-Za-z0-9_])"',
                    flags=default_flag)),
                ('AMEX', re.compile(
                    r'r"(?<![A-Za-z0-9_])(3[47]\d{13}|3[47]\d{2}-\d{6}-\d{5}|3[47]\d{2} \d{6} \d{5})(?!(?:[ -]?\d)|[A-Za-z0-9_])"',
                    flags=default_flag)),
                ...
                ('Discover', re.compile(
                    ...
                    flags=default_flag)),
                ('JCB', re.compile(
                    r'r"\b((?:(?:2131|1800)(?:[ -]?\d){11})|(?:35(?:2[89]|[3-8]\d)(?:[ -]?\d){12}))\b(?![ -]?\d)"',
                    flags=default_flag)),
                ('Maestro', re.compile(
                    r'r"\b((?:5[0678]\d{2}|6013|6[237]\d{2})(?:[ -]?\d){12,15})\b(?![ -]?\d)"',
                    flags=default_flag)),
                ('UnionPay', re.compile(
